...
| Referee | `RefereeProcessor` | `runtime/bridge/referee.py` | BRIDGE-only. Scores the student's utterance against the scenario's actions with a strict `json_schema` call (`REFEREE_PROVIDER` / `REFEREE_MODEL` / `REFEREE_REASONING`, mirroring the patient's `LLM_*` trio), applies the scenario's point values, and emits `transcript_update{student}` / `action_detected` / `state_update` / `game_over` **directly** over the data channel. Fails open: any error scores the turn as no-detection |
