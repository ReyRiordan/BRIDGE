...
`REFEREE_PROVIDER` / `REFEREE_MODEL` / `REFEREE_REASONING` are the referee's LLM trio, named to match the patient agent's `LLM_PROVIDER` / `LLM_MODEL` / `LLM_REASONING` (the kit's pipeline LLM). Same three concepts, different call.
