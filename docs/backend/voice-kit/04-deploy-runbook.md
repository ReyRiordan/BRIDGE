...
## 2. Local build smoke (the pre-deploy gate)
...
Watch for the AgentCore runtime reaching **READY** in the console.
...
- Teardown: destroy the stack (`npx ampx sandbox delete` or pipeline teardown). The KVS channel deletes with it; ECR images may linger per your retention.
